export const validate = (payload, setInvalidFields) => {
  ...
  let min = 6, max = 20, phoneNumber = 10


  let errors = new Set()   // Set để duy trì danh sách lỗi duy nhất
//   check rỗng

  fields.forEach((item) => { // map qua từng phần tử =>> mỗi pt ở đây vẫn là 1 mảng nhỏ
    // console.log("map",item)
    if (item[1] === "") {  
      setInvalidFields((prev) => [ // prev ở đây là invalidField trước nếu có => tức là nếu có lỗi trước đó thì giải ra và giữ lại sau đó + thêm lỗi tiếp
    ...
    switch (item[0]) {
      case "password":
        if (item[1].length < min) { // min = 6
          ...
          invalid++;
        } else if (item[1].length > max) {  // max = 20
          setInvalidFields((prev) => [
            ...prev,
            {
              name: item[0],
